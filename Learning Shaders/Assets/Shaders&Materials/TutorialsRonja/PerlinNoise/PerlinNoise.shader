Shader "Tutorial/PerlinNoise"
{
	...
	SubShader{
		...
		void surf(Input i, inout SurfaceOutputStandard o)
		{
			...
			//float pixHeight = fwidth(i.worldPos.y);
			//float lineIntens = smoothstep(2* pixHeight, pixHeight, dist);

			//float2 value = i.worldPos.xy / _CellSize;
			//float noise = perlinNoise(value) + 0.5;

			float3 value = i.worldPos.xyz / _CellSize;
			float3 noise = perlinNoise(value) + 0.5;
